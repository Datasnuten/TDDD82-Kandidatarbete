...
The algorithm works as follows. First, every streamer and point of interest is an object in a list. Second, a center of all points is calculated from all the points. This is done checking each and every objects latitude and longitude position and take the maximum and minimum of them. When having maximum and minimum longitude and latitude the algorithm will calculate the center of x-axis and y-axis as follows:
...
Third, now when we have the center point of all points we can calculate the maximum radius that everything will scale as follows:
...
For our calculations here we use the equiretangular approximation formula \cite{equi}:
