	...
	public class MediaPlayer extends TraitEventDispatcher
	{
	    ...
	    */
	    ...
	    public function seek(time:Number):void
	    {
			...
			/*CONFIG::LOGGING
								{
									logger.debug("branchleft hastrait " + (MediaTraitType.SEEK));
//									logger.debug("branchleft hastrait " + ((MediaTraitType.SEEK) as DynamicSeekTrait));
									logger.debug("branchleft hastrait " + media.hasTrait(MediaTraitType.SEEK));
								}*/
...
//			CONFIG::LOGGING
//			{
//				logger.debug("branchleft run main routine");
//			}
	    	return canSeek ? (getTraitOrThrow(MediaTraitType.SEEK) as SeekTrait).canSeekTo(time) : false; //false
	    }
...
//			CONFIG::LOGGING
//			{
//				logger.debug("PATPUT number of bytes loaded in media player: " + bytes);
//			}
	    ...
	    private function getTraitOrThrow(traitType:String):MediaTraitBase
	    {
	    	if (!media || !media.hasTrait(traitType)) 
	    	{
				CONFIG::LOGGING
				{
					logger.debug("branchleft hastrait " + media.hasTrait(traitType));
					logger.debug("branchleft hastrait " + media);
				}
...
//				CONFIG::LOGGING
//				{
//					logger.debug("branchleft nomedia or no hastrait " + media.getTrait(traitType));
//				}
...
//			{
//				logger.debug("branchleft gettrait "+ (media.getTrait(traitType)as SeekTrait));
//			}
		...
		CONFIG::LOGGING
		{
			private static const logger:Logger = Log.getLogger("org.osmf.media.MediaPlayer");
		}
	}
}
