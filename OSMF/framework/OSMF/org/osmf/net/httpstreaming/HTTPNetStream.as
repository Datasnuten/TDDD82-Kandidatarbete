...
package org.osmf.net.httpstreaming
{
	...
	public class HTTPNetStream extends NetStream
	{
		...
		protected function myKeyDown(event:KeyboardEvent):void
		{
			logger.debug("PREZ Pressed a key and noticed it!!!");
		}
		...
		public function bufferTimeChange(value:Number):void
		{
			_desiredBufferTime_Min =  value;
			_desiredBufferTime_Max = _desiredBufferTime_Min + OSMFSettings.hdsAdditionalBufferTime;
		}
		...
		private function onNetStatus(event:NetStatusEvent):void
		{
			CONFIG::LOGGING
			{
				logger.debug("NetStatus event:" + event.info.code + " "+ _wasBufferEmptied);
			}
...
//		Used to track player path (as fragments played) and helps deciding when and where to jump
		public function setpath(time:Number):void
		{
		if(time - last_time >= 1){
			var just_string:String = time.toString();
			var array:Array = just_string.split(".");
			var holding:uint = Math.round(time/4);
			if(previous_round != holding && array.length ==2 && array[1].length <= 3){
				if(check(holding)==true){
					logger.debug("set frag as "+ holding);
					last_time=time;
					previous_round=holding;
					{
						filedown.getFragment(holding);
						filedown.setCurrentFrag(holding);
					}
				}
			}
		}
		...
		private function check(hold:int):Boolean
		{
			if(hold == 25 && previous_round==5)
				return false;
			return true;
		}
				...
				case HTTPStreamingState.PLAY:
					...
					{						
						if (processed > 0)
						{
							CONFIG::LOGGING
							{
								logger.debug("bytes ( buffer = " + this.bufferLength + " )" ); 
							}
...
//		private function checkIfExtraBufferingNeeded():Boolean
		public function checkIfExtraBufferingNeeded():Boolean
		{
		 ...
		 */
		private function checkifdecisionpoint(lastfrag:String):Boolean
		{
			var anotherpath:String = lastfrag.substr(1);
			var patharray:Array = anotherpath.split(" ");
			var fragcheck:String = patharray[patharray.length-1];
			for(var i:uint=0; i < filedown.decPoints.length; i++){
				if(Number(fragcheck) == filedown.decPoints[i]){
					return true;
				}
			}
			return false;
		...
		private function checkiflandpoint(lastfrag:String,path:String):Boolean
		{
			for(var i:uint=0; i < filedown.landPoints.length; i++){
				if(Number(lastfrag) == filedown.landPoints[i]){
					return true;
				}
			}
			...
			var patharray:Array = anotherpath.split(" ");
			var fragcheck:String = patharray[patharray.length-1];
			if(Number(lastfrag) == Number(fragcheck)+1)
				return true;
			return false;
		}
		
		private function updatepath(playpath:String):void{
			filedown.receiveInfo(playpath);
		}
		
//		Decrease buffer upon buffering a new fragment, also helps with playpath tracking
		private function onBeginFragment(event:HTTPStreamingEvent):void
		{
			CONFIG::LOGGING
			{
				try
				{
					var max:int = _videoHandler.qosInfo.lastFragmentDetails.fragmentIdentifier.indexOf("Frag");
					var lastfragment:String = _videoHandler.qosInfo.lastFragmentDetails.fragmentIdentifier.substring(max+4);
					logger.debug("playpath Detected begin fragment for stream " + _videoHandler.qosInfo.lastFragmentDetails.fragmentIdentifier+ " "+ lastfragment);
					filedown.decreaseBuffer(Number(lastfragment));
					if(prevFrag != lastfragment && checkifdecisionpoint(playpath) == false){
						var concatdummy:String = " ";		
						concatdummy = concatdummy.concat(lastfragment);
						playpath = playpath.concat(concatdummy);
						concatdummy = " ";
						logger.debug("playpath"+ playpath);	
						prevFrag = lastfragment;
						updatepath(playpath);
					}
					
					if(prevFrag != lastfragment && checkifdecisionpoint(playpath) == true && checkiflandpoint(lastfragment,playpath) == true){
						var concatdummy1:String = " ";		
						concatdummy1 = concatdummy1.concat(lastfragment);
						playpath = playpath.concat(concatdummy1);
						concatdummy1 = " ";
						logger.debug("playpath"+ playpath);	
						prevFrag = lastfragment;
						updatepath(playpath);
		...
		private function onEndFragment(event:HTTPStreamingEvent):void
		{
			CONFIG::LOGGING
			{
				logger.debug("playpath Reached end fragment for stream " + _videoHandler.qosInfo.lastFragmentDetails.fragmentIdentifier );
			}
...
//		}
		...
		private var _videoHandler:IHTTPStreamHandler = null; //private originally
		...
		private var previous_round:uint=0;
