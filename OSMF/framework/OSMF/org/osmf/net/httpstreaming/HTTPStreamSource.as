...
package org.osmf.net.httpstreaming
{
	...
	public class HTTPStreamSource implements IHTTPStreamSource, IHTTPStreamHandler
	{
		...
		//######################################################################################################
		// Added for printing buffers from the downloader
		//######################################################################################################
		public function get bufferInfo():String
		{
			if(_downloader)
				return _downloader.bufferInfo;
			return "";
		}
		...
		public function seek(offset:Number):void
		{
			...
			CONFIG::LOGGING
			{			
				logger.debug("set state as seek");
			}
		}
		...
		protected function doSomeProcessingAndGetBytes():ByteArray
		{
			...
			switch(_state)
			{
				...
				case HTTPStreamingState.LOAD:
					...
					{
//						_indexDownloader.getSecBytes(_indexDownloader.downloadSecBytesCount);
						_request = _indexHandler.getFileForTime(_seekTarget, _qualityLevel);
						logger.debug("seektarget "+ _request);
						wasSeek = true;
					}
					else
					{
					...
					//##############################################################################
					// Added for jumping branches
					//#############################################################################
					var lastNums:Number;
					var lastNum:String;
					var lengthOfReq:uint;
					if(_request.url)
					{
						// Parses the request url for the fragment number
						lengthOfReq = _request.url.length;
						while(!isNaN(Number(_request.url.charAt(lengthOfReq))))
						{
							if(isNaN(Number(_request.url.charAt(lengthOfReq))))
								break;
							lengthOfReq--;
						}
						lastNum = _request.url.substr(lengthOfReq+1);
						lastNums = Number(lastNum);
					}
					...
					switch(_request.kind)
					{
						...
						case HTTPStreamRequestKind.BEST_EFFORT_DOWNLOAD:
							...
							}
							{
								CONFIG::LOGGING
								{			
									logger.debug("downloader.open "+_request.urlRequest.url + " "+ lastNums);
								}
								_downloader.open(_request.urlRequest, downloaderMonitor, OSMFSettings.hdsFragmentDownloadTimeout,1);
				...
				case HTTPStreamingState.BEGIN_FRAGMENT:
					if(_request.kind == HTTPStreamRequestKind.BEST_EFFORT_DOWNLOAD)
					{
						...
						{
							// index handler wants us to process thi s fragment.
							// fall through
						}
						else
						{
				...
				case HTTPStreamingState.END_FRAGMENT:
					...
					var lastQuality:String = requestURL.substring((requestURL.lastIndexOf("_"))+1,requestURL.lastIndexOf("Seg"));
					downManager.receiveFraginfo(fragmentIdentifier,lastQuality);
		...
		{
			var landPoint:Vector.<Number> = downManager.landPoints;
		...
		public function checkparalleldownload(fragno:uint):Boolean
		{
			var downloaded:Array = downManager.returnPrefetchpath();
			for (var i:uint=0; i< downloaded.length; i++){
				if(fragno == downloaded[i])
					return true;
			}
			return false;
		}
		...
		private function onRequestLoadIndex(event:HTTPStreamingIndexHandlerEvent):void
		{
			...
			if (_currentIndexDownloadEvent == null)
			{
				...
				_indexDownloader.open(_currentIndexDownloadEvent.request, _indexDownloaderMonitor, OSMFSettings.hdsIndexDownloadTimeout,2);
			}
		}
...
//			if(_indexDownloader.getFragCount() != 8)
				_indexDownloader.getBytes(_indexDownloader.downloadBytesCount);
//			else
//				_indexDownloader.getSecBytes(_indexDownloader.downloadSecBytesCount);
		...
		private function processPendingIndexLoadingRequest():void
		{
			...
			else
			{
				...
				_indexDownloader.open(_currentIndexDownloadEvent.request, _indexDownloaderMonitor, OSMFSettings.hdsIndexDownloadTimeout,4);
			}	
		}
		...
		private var _request_alt:HTTPStreamRequest = null;
