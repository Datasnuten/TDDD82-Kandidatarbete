...
package org.osmf.net.httpstreaming.f4f
{
	...
	internal class AdobeSegmentRunTable extends FullBox
	{
		...
		public function get totalFragments():uint
		{
			...
			CONFIG::LOGGING
			{
				for (var i:uint = 0; i < _segmentFragmentPairs.length; i++)
				{
					logger.debug("FRAGPA number of segments: " + _segmentFragmentPairs.length);
				}
				
				logger.debug("FRAGPA total fragments: " + (_segmentFragmentPairs[_segmentFragmentPairs.length - 1].fragmentsPerSegment + _segmentFragmentPairs[_segmentFragmentPairs.length - 1].fragmentsAccrued) );
			}
		...
		private function calculateSegmentId(sfp:SegmentFragmentPair, fragmentId:uint):uint
		{
			...
			CONFIG::LOGGING
			{
				...
				logger.debug("FRIDPA fragId: " + fragmentId);
