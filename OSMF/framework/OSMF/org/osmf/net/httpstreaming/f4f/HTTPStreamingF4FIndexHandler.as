	...
	public class HTTPStreamingF4FIndexHandler extends HTTPStreamingIndexHandlerBase
	{
		...
		private function fragmentOverflow(bootstrapBox:AdobeBootstrapBox, fragId:uint):Boolean
		{
			...
			for (var i:uint = 0; i <= 100; i++){
				filedown.runtable[i] = fragmentRunTable.fragmentDurationPairs[i].duration;
			}
			filedown.calculatefragSize();
		...
		{
//			logger.debug("when do i run "+ nextFragmentId);
			// if we had a pending BEF download, invalidate it
			stopListeningToBestEffortDownload();
...
//		private var filedown:HTTPDownloadManager = null;
		private var filedown:HTTPDownloadManager = HTTPDownloadManager.getInstance();
