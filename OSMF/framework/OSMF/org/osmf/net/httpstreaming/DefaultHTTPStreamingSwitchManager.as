...
package org.osmf.net.httpstreaming
{
	...
	public class DefaultHTTPStreamingSwitchManager extends RuleSwitchManagerBase
	{
		 ...
		 */
		//		added
		public static var bitRate:uint = 0;
		//		added
		...
		protected function getMaxIndex(maxBitrate:Number, emergencyFlag:Boolean = false):uint
		{
			...
			// ADDED			//////////////////////////////////////////////////////////////////////
			if(holding != 0 && maxBitrate != 0 && Math.abs(maxBitrate-holding) > 10000){ //>10000 used to prevent spikes observed at times
				maxBitrate = holding;
			}
			CONFIG::LOGGING
			{
			...
			downManager.pushtoStreamdownloader(maxBitrate,newQuality.bitrate);
			var setQas:Number=0.25;	
			if(newQuality.bitrate < maxBitrate)
			{
				var share1:uint = maxBitrate-newQuality.bitrate;
				logger.debug("share1 "+share1);
				var share:Number = share1/1000;
				if(share > 0){
					var defq:Array = [0.25, 0.5, 0.85, 1.3];
					var thisq:Number = 0;
					if(share <= defq[0])
						setQas = 0.25;
					for(var j:uint=0; j < defq.length; j++ ){
						if(share > defq[j]){
							thisq = defq[j];
						}
					}
					...
					downManager.setprefetchquality(setQas);
				}
			}
			else{
				downManager.setprefetchquality(0.25);
			}	
			return newQuality.index;
		}
		
		public static function givebitRate():uint
		{
			return bitRate;
		}
		...
		private var alreadychecked:Array=new Array;
		private var previousprefetch:Array = new Array;
		private var prefetchedbefore:Boolean = false;
		private var prefetchedbeforequality:Number=0;
		private var holding:uint = 0;
