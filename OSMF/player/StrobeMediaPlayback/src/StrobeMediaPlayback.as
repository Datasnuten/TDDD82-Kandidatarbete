...
{	
	...
	public class StrobeMediaPlayback extends Sprite
	{
		...
		{			
			...
			var myTimer1:Timer = new Timer(500, 0); // 1 second
			myTimer1.addEventListener(TimerEvent.TIMER, automate);
			myTimer1.start();
			...
			fileDown.addEventListener(Event.COMPLETE, updateDownloadManager);
			fileDown.addEventListener("showAlert", addAlert);
			fileDown.addEventListener("closeAlert", closeAlert);
			CONFIG::LOGGING
			{
		...
		//##### ADDED #####
		private function updateDownloadManager(event:Event):void
		{
			mediaContainer.addMediaElement(alert);
		//	alert.tintColor = 0;
		//	alert.alert("Text", fileDown.getMessage());
		}
		private function keyTest(event:KeyboardEvent):void
		{
			MultiJumpButton.getInstance().processKey(event.charCode);	// After getting a keyboard event, passes the data to the button
		}
		
		private function addAlert(event:Event):void
		{
		//	mediaContainer.addMediaElement(alert);
			alert.tintColor = 0xFAFAFA; 			// This should change the colour but it doesn't!! GRrr
			alert.alert("Text", fileDown.getMessage());
			var time:Timer = new Timer(3000, 1);
			time.addEventListener(TimerEvent.TIMER, closeAlert);
			time.start();
		}
		...
		private function automate(event:TimerEvent):void
		{
			var timeTojump:Array = fileDown.jumpTime;
			var segTojump:Array  = fileDown.pushButton;
//			if(fileDown.timePass>85)
//				logger.debug("");
			for(var i:uint=0;i<timeTojump.length;i++){
				if(fileDown.timePass >1 && fileDown.timePass > timeTojump[i] && done.indexOf(timeTojump[i]) < 0){
					MultiJumpButton.getInstance().processKey((Number(segTojump[i])+48));
					done.push(timeTojump[i]);
				}
			}
		}
		...
		private var done:Array=new Array;
		// ##### ADDED ######
